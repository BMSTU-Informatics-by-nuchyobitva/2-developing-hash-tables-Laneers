...
class HashTable {
	...
	//size_t Hash(std::string key) const{ // хэш-функция
	//	size_t hash = 0;
	//	size_t high;
	//	for (char c : key) {
	//		hash = (hash << 4) + c; // cдвигаем хэш влево и добавляем символ
	//		high = hash & 0xF0000000; // берем старшие 4 бита
	//		if (high) {
	//			hash ^= high >> 24; // xor-им старшие биты с младшими
	//		}
	//		hash &= ~high; // обнуляем старшие биты
	//	}
	//	return hash % capacity;
	//}
